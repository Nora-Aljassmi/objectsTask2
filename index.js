...
//   return student.courses.find((major) => {
//     return major == student.courses[courseIndex];
//   });
// // // }
function getCourse(student, courseIndex) {
...
console.log(countCourses(students[6]));
...
function listAllCourses(students) {
  const allcourses = [];
  //merging all courses in one array
  students.forEach((objs2) => {
    allcourses.push(...objs2.courses);
  });
  //end
  // returning non duplicated array
  return allcourses.filter((flt, index) => {
    return allcourses.indexOf(flt) == index;
  });
  ///end
}
...
function removeCourseFromStudent(student, course) {
  const filtArr = student.courses.filter((crs) => {
    return crs != course;
  });
  student.courses = filtArr;
  return student;
}
...
function getStudentsByCourse(course, students) {
  return students.filter((objs) => {
    return objs.courses.find((elem) => {
      return elem == course;
    });
  });
  // return students.filter((objs) => objs.courses.find((elem) => elem == course));
}
